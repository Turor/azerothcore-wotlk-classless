...
void Player::UpdateAttackPowerAndDamage(bool ranged) {
    ...
    if (ranged) {
        ...
        if (IsClass(CLASS_PALADIN, CLASS_CONTEXT_STATS) || IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_STATS) || IsClass(
                CLASS_WARRIOR, CLASS_CONTEXT_STATS)) {
            baseAttackPower = level * 3.0f + GetStat(STAT_STRENGTH) * 2.0f - 20.0f;
        } else if (IsClass(CLASS_HUNTER, CLASS_CONTEXT_STATS) || IsClass(CLASS_SHAMAN, CLASS_CONTEXT_STATS) || IsClass(
                       CLASS_ROGUE, CLASS_CONTEXT_STATS)) {
            ...
            switch (GetShapeshiftForm()) {
                case FORM_CAT:
                    baseAttackPower = (GetLevel() * mLevelMult) + GetStat(STAT_STRENGTH) * 2.0f + GetStat(STAT_AGILITY)
                                      - 20.0f + weapon_bonus + m_baseFeralAP;
                    break;
                case FORM_BEAR:
                case FORM_DIREBEAR:
                    baseAttackPower = (GetLevel() * mLevelMult) + GetStat(STAT_STRENGTH) * 2.0f - 20.0f + weapon_bonus +
                                      m_baseFeralAP;
                    break;
                case FORM_MOONKIN:
        ...
        } else if (IsClass(CLASS_MAGE, CLASS_CONTEXT_STATS) || IsClass(CLASS_PRIEST, CLASS_CONTEXT_STATS) || IsClass(
                       CLASS_WARLOCK, CLASS_CONTEXT_STATS)) {
            baseAttackPower = GetStat(STAT_STRENGTH) - 10.0f;
        }
    }

    SetStatFlatModifier(unitMod, BASE_VALUE, baseAttackPower);
    ...
    sScriptMgr->OnPlayerAfterUpdateAttackPowerAndDamage(this, level, base_attPower, attPowerMod, attPowerMultiplier,
                                                        ranged);
    ...
    if (ranged) {
        ...
        if (CanDualWield() && HasOffhandWeaponForAttack())
            //allow update offhand damage only if player knows DualWield Spec and has equipped offhand weapon
            UpdateDamagePhysical(OFF_ATTACK);
        if (IsClass(CLASS_SHAMAN, CLASS_CONTEXT_STATS) || IsClass(CLASS_PALADIN, CLASS_CONTEXT_STATS))
            // mental quickness
            UpdateSpellDamageAndHealingBonus();
    }
}
...
void Player::CalculateMinMaxDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, float &minDamage,
                                   float &maxDamage, uint8 damageIndex) {
    ...
    float baseValue = GetFlatModifierValue(unitMod, BASE_VALUE) + GetTotalAttackPowerValue(attType) / 14.0f *
                      attackSpeedMod;
...
float Player::GetMissPercentageFromDefence() const {
    ...
    return nondiminishing + (diminishing * miss_cap[pclass] / (
...
void Player::UpdateParryPercentage() {
    ...
    if (CanParry() && parry_cap[pclass] > 0.0f) {
        ...
        m_realParry = nondiminishing + diminishing * parry_cap[pclass] / (
...
void Player::UpdateDodgePercentage() {
    ...
    uint32 pclass = getClass() - 1;
    m_realDodge = nondiminishing + (diminishing * dodge_cap[pclass] / (
...
void Player::UpdateManaRegen() {
    ...
    float power_regen_mp5 = (GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_POWER_REGEN, POWER_MANA) + m_baseManaRegen)
                            / 5.0f;
...
void Player::UpdateEnergyRegen() {
    ...
    regenPerSecond += static_cast<float>(GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_POWER_REGEN, POWER_ENERGY)) /
            static_cast<float>((5 * IN_MILLISECONDS));

    SetStatFloatValue(UNIT_FIELD_POWER_REGEN_FLAT_MODIFIER + AsUnderlyingType(POWER_ENERGY), regenPerSecond - 10.f);
    SetStatFloatValue(UNIT_FIELD_POWER_REGEN_INTERRUPTED_FLAT_MODIFIER + AsUnderlyingType(POWER_ENERGY),
                      regenPerSecond - 10.f);
}
...
void Creature::UpdateAttackPowerAndDamage(bool ranged) {
    ...
    SetInt32Value(index, uint32(base_attPower)); // UNIT_FIELD_(RANGED)_ATTACK_POWER
    SetInt32Value(indexMod, uint32(attPowerMod)); // UNIT_FIELD_(RANGED)_ATTACK_POWER_MODS
...
void Creature::CalculateMinMaxDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, float &minDamage,
    ...
    float base_value  = GetFlatModifierValue(unitMod, BASE_VALUE) + GetTotalAttackPowerValue(attType) / 14.0f * attackSpeedMulti;
    ...
    minDamage = ((weaponMinDamage + base_value) * dmgMultiplier * base_pct + total_value) * total_pct;
    maxDamage = ((weaponMaxDamage + base_value) * dmgMultiplier * base_pct + total_value) * total_pct;
...
void Guardian::UpdateMaxPower(Powers power) {
    ...
    float addValue = (power == POWER_MANA)
                         ? std::max<float>(GetStat(STAT_INTELLECT) - GetCreateStat(STAT_INTELLECT), 0.0f)
                         : 0.0f;
